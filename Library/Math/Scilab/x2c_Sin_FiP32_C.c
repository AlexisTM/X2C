...
/* */
void x2c_Sin_FiP32_C(scicos_block* block, scicos_flag flag);
static void x2c_Sin_FiP32_C__OutputUpdate(scicos_block* block);
static void x2c_Sin_FiP32_C__StateUpdate(scicos_block* block);
static void x2c_Sin_FiP32_C__Initialization(scicos_block* block);
static void x2c_Sin_FiP32_C__Ending(scicos_block* block);
...
void x2c_Sin_FiP32_C(scicos_block* block, scicos_flag flag) {
    ...
    switch (flag) {
        ...
        case OutputUpdate:     /* 1 */
        {
            x2c_Sin_FiP32_C__OutputUpdate(block);
            break;
        }
        case StateUpdate:      /* 2 */
        {
            x2c_Sin_FiP32_C__StateUpdate(block);
            break;
        }
        ...
        {
            x2c_Sin_FiP32_C__Initialization(block);
            break;
        }
        case Ending:           /* 5 */
        {
            x2c_Sin_FiP32_C__Ending(block);
            break;
        }
...
static void x2c_Sin_FiP32_C__set_parameter(scicos_block* block, SIN_FIP32 *blockParam) {
    int *paramI;
    double *paramR;
    SIN_FIP32 *pTSin_FiP32;
    
    /* *** */
    paramI = GetIparPtrs(block);  /* integer parameters  */
    paramR = GetRparPtrs(block);  /* real parameters     */
    pTSin_FiP32 = GetWorkPtrs(block);
}
...
static void x2c_Sin_FiP32_C__Initialization(scicos_block* block) {
    ...
    /* *** */
    pTSin_FiP32 = scicos_malloc(sizeof(SIN_FIP32));
    GetWorkPtrs(block) = pTSin_FiP32;
    x2c_Sin_FiP32_C__set_parameter(block, pTSin_FiP32);

    /* Assignment of inports */
    inport_In_p = GetInPortPtrs(block, 1);
    inport_In = convert_double_to_Q31(*inport_In_p);
    pTSin_FiP32->In = &inport_In;

    /* Assignment of parameter arrays */

    /* Initialization */
    Sin_FiP32_Init(pTSin_FiP32);

    /* Assignment of outports */
    outport_Out = GetOutPortPtrs(block, 1);
    *outport_Out = convert_Q31_to_double(pTSin_FiP32->Out);
}
...
static void x2c_Sin_FiP32_C__Ending(scicos_block* block) {
    SIN_FIP32 *pTSin_FiP32;
    pTSin_FiP32 = GetWorkPtrs(block);
    scicos_free(pTSin_FiP32);
}


static void x2c_Sin_FiP32_C__StateUpdate_f(scicos_block* block) {
...
static void x2c_Sin_FiP32_C__StateUpdate(scicos_block* block) {
    if (GetNevIn(block) > 0) {
        ...
        */
        x2c_Sin_FiP32_C__StateUpdate_f(block);
    }
}
