...
/* */
void x2c_Sqrt_FiP8_C(scicos_block* block, scicos_flag flag);
static void x2c_Sqrt_FiP8_C__OutputUpdate(scicos_block* block);
static void x2c_Sqrt_FiP8_C__StateUpdate(scicos_block* block);
static void x2c_Sqrt_FiP8_C__Initialization(scicos_block* block);
static void x2c_Sqrt_FiP8_C__Ending(scicos_block* block);
...
void x2c_Sqrt_FiP8_C(scicos_block* block, scicos_flag flag) {
    ...
    switch (flag) {
        ...
        case OutputUpdate:     /* 1 */
        {
            x2c_Sqrt_FiP8_C__OutputUpdate(block);
            break;
        }
        case StateUpdate:      /* 2 */
        {
            x2c_Sqrt_FiP8_C__StateUpdate(block);
            break;
        }
        ...
        {
            x2c_Sqrt_FiP8_C__Initialization(block);
            break;
        }
        case Ending:           /* 5 */
        {
            x2c_Sqrt_FiP8_C__Ending(block);
            break;
        }
...
static void x2c_Sqrt_FiP8_C__set_parameter(scicos_block* block, SQRT_FIP8 *blockParam) {
    int *paramI;
    double *paramR;
    SQRT_FIP8 *pTSqrt_FiP8;
    
    /* *** */
    paramI = GetIparPtrs(block);  /* integer parameters  */
    paramR = GetRparPtrs(block);  /* real parameters     */
    pTSqrt_FiP8 = GetWorkPtrs(block);
}
...
static void x2c_Sqrt_FiP8_C__Initialization(scicos_block* block) {
    ...
    /* *** */
    pTSqrt_FiP8 = scicos_malloc(sizeof(SQRT_FIP8));
    GetWorkPtrs(block) = pTSqrt_FiP8;
    x2c_Sqrt_FiP8_C__set_parameter(block, pTSqrt_FiP8);

    /* Assignment of inports */
    inport_In_p = GetInPortPtrs(block, 1);
    inport_In = convert_double_to_Q7(*inport_In_p);
    pTSqrt_FiP8->In = &inport_In;

    /* Assignment of parameter arrays */

    /* Initialization */
    Sqrt_FiP8_Init(pTSqrt_FiP8);

    /* Assignment of outports */
    outport_Out = GetOutPortPtrs(block, 1);
    *outport_Out = convert_Q7_to_double(pTSqrt_FiP8->Out);
}
...
static void x2c_Sqrt_FiP8_C__Ending(scicos_block* block) {
    SQRT_FIP8 *pTSqrt_FiP8;
    pTSqrt_FiP8 = GetWorkPtrs(block);
    scicos_free(pTSqrt_FiP8);
}


static void x2c_Sqrt_FiP8_C__StateUpdate_f(scicos_block* block) {
...
static void x2c_Sqrt_FiP8_C__StateUpdate(scicos_block* block) {
    if (GetNevIn(block) > 0) {
        ...
        */
        x2c_Sqrt_FiP8_C__StateUpdate_f(block);
    }
}
