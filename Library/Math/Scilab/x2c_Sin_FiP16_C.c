...
/* */
void x2c_Sin_FiP16_C(scicos_block* block, scicos_flag flag);
static void x2c_Sin_FiP16_C__OutputUpdate(scicos_block* block);
static void x2c_Sin_FiP16_C__StateUpdate(scicos_block* block);
static void x2c_Sin_FiP16_C__Initialization(scicos_block* block);
static void x2c_Sin_FiP16_C__Ending(scicos_block* block);
...
void x2c_Sin_FiP16_C(scicos_block* block, scicos_flag flag) {
    ...
    switch (flag) {
        ...
        case OutputUpdate:     /* 1 */
        {
            x2c_Sin_FiP16_C__OutputUpdate(block);
            break;
        }
        case StateUpdate:      /* 2 */
        {
            x2c_Sin_FiP16_C__StateUpdate(block);
            break;
        }
        ...
        {
            x2c_Sin_FiP16_C__Initialization(block);
            break;
        }
        case Ending:           /* 5 */
        {
            x2c_Sin_FiP16_C__Ending(block);
            break;
        }
...
static void x2c_Sin_FiP16_C__set_parameter(scicos_block* block, SIN_FIP16 *blockParam) {
    int *paramI;
    double *paramR;
    SIN_FIP16 *pTSin_FiP16;
    
    /* *** */
    paramI = GetIparPtrs(block);  /* integer parameters  */
    paramR = GetRparPtrs(block);  /* real parameters     */
    pTSin_FiP16 = GetWorkPtrs(block);
}
...
static void x2c_Sin_FiP16_C__Initialization(scicos_block* block) {
    ...
    /* *** */
    pTSin_FiP16 = scicos_malloc(sizeof(SIN_FIP16));
    GetWorkPtrs(block) = pTSin_FiP16;
    x2c_Sin_FiP16_C__set_parameter(block, pTSin_FiP16);

    /* Assignment of inports */
    inport_In_p = GetInPortPtrs(block, 1);
    inport_In = convert_double_to_Q15(*inport_In_p);
    pTSin_FiP16->In = &inport_In;

    /* Assignment of parameter arrays */

    /* Initialization */
    Sin_FiP16_Init(pTSin_FiP16);

    /* Assignment of outports */
    outport_Out = GetOutPortPtrs(block, 1);
    *outport_Out = convert_Q15_to_double(pTSin_FiP16->Out);
}
...
static void x2c_Sin_FiP16_C__Ending(scicos_block* block) {
    SIN_FIP16 *pTSin_FiP16;
    pTSin_FiP16 = GetWorkPtrs(block);
    scicos_free(pTSin_FiP16);
}


static void x2c_Sin_FiP16_C__StateUpdate_f(scicos_block* block) {
...
static void x2c_Sin_FiP16_C__StateUpdate(scicos_block* block) {
    if (GetNevIn(block) > 0) {
        ...
        */
        x2c_Sin_FiP16_C__StateUpdate_f(block);
    }
}
