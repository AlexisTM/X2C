...
/* */
void x2c_uConstant_FiP32_C(scicos_block* block, scicos_flag flag);
static void x2c_uConstant_FiP32_C__OutputUpdate(scicos_block* block);
static void x2c_uConstant_FiP32_C__StateUpdate(scicos_block* block);
static void x2c_uConstant_FiP32_C__Initialization(scicos_block* block);
static void x2c_uConstant_FiP32_C__Ending(scicos_block* block);
...
void x2c_uConstant_FiP32_C(scicos_block* block, scicos_flag flag) {
    ...
    switch (flag) {
        ...
        case OutputUpdate:     /* 1 */
        {
            x2c_uConstant_FiP32_C__OutputUpdate(block);
            break;
        }
        case StateUpdate:      /* 2 */
        {
            x2c_uConstant_FiP32_C__StateUpdate(block);
            break;
        }
        ...
        {
            x2c_uConstant_FiP32_C__Initialization(block);
            break;
        }
        case Ending:           /* 5 */
        {
            x2c_uConstant_FiP32_C__Ending(block);
            break;
        }
...
static void x2c_uConstant_FiP32_C__set_parameter(scicos_block* block, UCONSTANT_FIP32 *blockParam) {
    int *paramI;
    double *paramR;
    UCONSTANT_FIP32 *pTuConstant_FiP32;
    
    /* *** */
    paramI = GetIparPtrs(block);  /* integer parameters  */
    paramR = GetRparPtrs(block);  /* real parameters     */
    pTuConstant_FiP32 = GetWorkPtrs(block);
    blockParam->K = (int32)paramI[0];
}
...
static void x2c_uConstant_FiP32_C__Initialization(scicos_block* block) {
    ...
    /* *** */
    pTuConstant_FiP32 = scicos_malloc(sizeof(UCONSTANT_FIP32));
    GetWorkPtrs(block) = pTuConstant_FiP32;
    x2c_uConstant_FiP32_C__set_parameter(block, pTuConstant_FiP32);
    ...
    /* Initialization */
    uConstant_FiP32_Init(pTuConstant_FiP32);

    /* Assignment of outports */
    outport_Out = GetOutPortPtrs(block, 1);
    *outport_Out = convert_Q31_to_double(pTuConstant_FiP32->Out);
}
...
static void x2c_uConstant_FiP32_C__Ending(scicos_block* block) {
    UCONSTANT_FIP32 *pTuConstant_FiP32;
    pTuConstant_FiP32 = GetWorkPtrs(block);
    scicos_free(pTuConstant_FiP32);
}


static void x2c_uConstant_FiP32_C__StateUpdate_f(scicos_block* block) {
...
static void x2c_uConstant_FiP32_C__StateUpdate(scicos_block* block) {
    if (GetNevIn(block) > 0) {
        ...
        */
        x2c_uConstant_FiP32_C__StateUpdate_f(block);
    }
}
