...
/* */
void x2c_SaveSignal_FiP8_C(scicos_block* block, scicos_flag flag);
static void x2c_SaveSignal_FiP8_C__OutputUpdate(scicos_block* block);
static void x2c_SaveSignal_FiP8_C__StateUpdate(scicos_block* block);
static void x2c_SaveSignal_FiP8_C__Initialization(scicos_block* block);
static void x2c_SaveSignal_FiP8_C__Ending(scicos_block* block);
...
void x2c_SaveSignal_FiP8_C(scicos_block* block, scicos_flag flag) {
    ...
    switch (flag) {
        ...
        case OutputUpdate:     /* 1 */
        {
            x2c_SaveSignal_FiP8_C__OutputUpdate(block);
            break;
        }
        case StateUpdate:      /* 2 */
        {
            x2c_SaveSignal_FiP8_C__StateUpdate(block);
            break;
        }
        ...
        {
            x2c_SaveSignal_FiP8_C__Initialization(block);
            break;
        }
        case Ending:           /* 5 */
        {
            x2c_SaveSignal_FiP8_C__Ending(block);
            break;
        }
...
static void x2c_SaveSignal_FiP8_C__set_parameter(scicos_block* block, SAVESIGNAL_FIP8 *blockParam) {
    int *paramI;
    double *paramR;
    SAVESIGNAL_FIP8 *pTSaveSignal_FiP8;
    
    /* *** */
    paramI = GetIparPtrs(block);  /* integer parameters  */
    paramR = GetRparPtrs(block);  /* real parameters     */
    pTSaveSignal_FiP8 = GetWorkPtrs(block);
}
...
static void x2c_SaveSignal_FiP8_C__Initialization(scicos_block* block) {
    ...
    /* *** */
    pTSaveSignal_FiP8 = scicos_malloc(sizeof(SAVESIGNAL_FIP8));
    GetWorkPtrs(block) = pTSaveSignal_FiP8;
    x2c_SaveSignal_FiP8_C__set_parameter(block, pTSaveSignal_FiP8);

    /* Assignment of inports */
    inport_In_p = GetInPortPtrs(block, 1);
    inport_In = convert_double_to_Q7(*inport_In_p);
    pTSaveSignal_FiP8->In = &inport_In;

    /* Assignment of parameter arrays */

    /* Initialization */
    SaveSignal_FiP8_Init(pTSaveSignal_FiP8);
...
static void x2c_SaveSignal_FiP8_C__Ending(scicos_block* block) {
    SAVESIGNAL_FIP8 *pTSaveSignal_FiP8;
    pTSaveSignal_FiP8 = GetWorkPtrs(block);
    scicos_free(pTSaveSignal_FiP8);
}


static void x2c_SaveSignal_FiP8_C__StateUpdate_f(scicos_block* block) {
...
static void x2c_SaveSignal_FiP8_C__StateUpdate(scicos_block* block) {
    if (GetNevIn(block) > 0) {
        ...
        */
        x2c_SaveSignal_FiP8_C__StateUpdate_f(block);
    }
}
