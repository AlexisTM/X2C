...
/* */
void x2c_Sin3Gen_FiP32_C(scicos_block* block, scicos_flag flag);
static void x2c_Sin3Gen_FiP32_C__OutputUpdate(scicos_block* block);
static void x2c_Sin3Gen_FiP32_C__StateUpdate(scicos_block* block);
static void x2c_Sin3Gen_FiP32_C__Initialization(scicos_block* block);
static void x2c_Sin3Gen_FiP32_C__Ending(scicos_block* block);
...
void x2c_Sin3Gen_FiP32_C(scicos_block* block, scicos_flag flag) {
    ...
    switch (flag) {
        ...
        case OutputUpdate:     /* 1 */
        {
            x2c_Sin3Gen_FiP32_C__OutputUpdate(block);
            break;
        }
        case StateUpdate:      /* 2 */
        {
            x2c_Sin3Gen_FiP32_C__StateUpdate(block);
            break;
        }
        ...
        {
            x2c_Sin3Gen_FiP32_C__Initialization(block);
            break;
        }
        case Ending:           /* 5 */
        {
            x2c_Sin3Gen_FiP32_C__Ending(block);
            break;
        }
...
static void x2c_Sin3Gen_FiP32_C__set_parameter(scicos_block* block, SIN3GEN_FIP32 *blockParam) {
    int *paramI;
    double *paramR;
    SIN3GEN_FIP32 *pTSin3Gen_FiP32;
    
    /* *** */
    paramI = GetIparPtrs(block);  /* integer parameters  */
    paramR = GetRparPtrs(block);  /* real parameters     */
    pTSin3Gen_FiP32 = GetWorkPtrs(block);
    blockParam->delta_phi = (int32)paramI[0];
    blockParam->offset = (int32)paramI[1];
}
...
static void x2c_Sin3Gen_FiP32_C__Initialization(scicos_block* block) {
    ...
    /* *** */
    pTSin3Gen_FiP32 = scicos_malloc(sizeof(SIN3GEN_FIP32));
    GetWorkPtrs(block) = pTSin3Gen_FiP32;
    x2c_Sin3Gen_FiP32_C__set_parameter(block, pTSin3Gen_FiP32);

    /* Assignment of inports */
    inport_A_p = GetInPortPtrs(block, 1);
    inport_A = convert_double_to_Q31(*inport_A_p);
    pTSin3Gen_FiP32->A = &inport_A;
    inport_f_p = GetInPortPtrs(block, 2);
    inport_f = convert_double_to_Q31(*inport_f_p);
    pTSin3Gen_FiP32->f = &inport_f;

    /* Assignment of parameter arrays */

    /* Initialization */
    Sin3Gen_FiP32_Init(pTSin3Gen_FiP32);

    /* Assignment of outports */
    outport_u = GetOutPortPtrs(block, 1);
    *outport_u = convert_Q31_to_double(pTSin3Gen_FiP32->u);
    outport_v = GetOutPortPtrs(block, 2);
    *outport_v = convert_Q31_to_double(pTSin3Gen_FiP32->v);
    outport_w = GetOutPortPtrs(block, 3);
    *outport_w = convert_Q31_to_double(pTSin3Gen_FiP32->w);
}
...
static void x2c_Sin3Gen_FiP32_C__Ending(scicos_block* block) {
    SIN3GEN_FIP32 *pTSin3Gen_FiP32;
    pTSin3Gen_FiP32 = GetWorkPtrs(block);
    scicos_free(pTSin3Gen_FiP32);
}


static void x2c_Sin3Gen_FiP32_C__StateUpdate_f(scicos_block* block) {
...
static void x2c_Sin3Gen_FiP32_C__StateUpdate(scicos_block* block) {
    if (GetNevIn(block) > 0) {
        ...
        */
        x2c_Sin3Gen_FiP32_C__StateUpdate_f(block);
    }
}
...
static void x2c_Sin3Gen_FiP32_C__OutputUpdate(scicos_block* block) {
    ...
    /* Update */
    Sin3Gen_FiP32_Update(pTSin3Gen_FiP32);
