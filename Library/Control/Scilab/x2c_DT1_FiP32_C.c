...
/* */
void x2c_DT1_FiP32_C(scicos_block* block, scicos_flag flag);
static void x2c_DT1_FiP32_C__OutputUpdate(scicos_block* block);
static void x2c_DT1_FiP32_C__StateUpdate(scicos_block* block);
static void x2c_DT1_FiP32_C__Initialization(scicos_block* block);
static void x2c_DT1_FiP32_C__Ending(scicos_block* block);
...
void x2c_DT1_FiP32_C(scicos_block* block, scicos_flag flag) {
    ...
    switch (flag) {
        ...
        case OutputUpdate:     /* 1 */
        {
            x2c_DT1_FiP32_C__OutputUpdate(block);
            break;
        }
        case StateUpdate:      /* 2 */
        {
            x2c_DT1_FiP32_C__StateUpdate(block);
            break;
        }
        ...
        {
            x2c_DT1_FiP32_C__Initialization(block);
            break;
        }
        case Ending:           /* 5 */
        {
            x2c_DT1_FiP32_C__Ending(block);
            break;
        }
...
static void x2c_DT1_FiP32_C__set_parameter(scicos_block* block, DT1_FIP32 *blockParam) {
    int *paramI;
    double *paramR;
    DT1_FIP32 *pTDT1_FiP32;
    
    /* *** */
    paramI = GetIparPtrs(block);  /* integer parameters  */
    paramR = GetRparPtrs(block);  /* real parameters     */
    pTDT1_FiP32 = GetWorkPtrs(block);
    blockParam->b0 = (int32)paramI[0];
    blockParam->b1 = (int32)paramI[1];
    blockParam->a0 = (int32)paramI[2];
    blockParam->sfrb = (int8)paramI[3];
    blockParam->sfra = (int8)paramI[4];
}
...
static void x2c_DT1_FiP32_C__Initialization(scicos_block* block) {
    ...
    /* *** */
    pTDT1_FiP32 = scicos_malloc(sizeof(DT1_FIP32));
    GetWorkPtrs(block) = pTDT1_FiP32;
    x2c_DT1_FiP32_C__set_parameter(block, pTDT1_FiP32);

    /* Assignment of inports */
    inport_In_p = GetInPortPtrs(block, 1);
    inport_In = convert_double_to_Q31(*inport_In_p);
    pTDT1_FiP32->In = &inport_In;

    /* Assignment of parameter arrays */

    /* Initialization */
    DT1_FiP32_Init(pTDT1_FiP32);

    /* Assignment of outports */
    outport_Out = GetOutPortPtrs(block, 1);
    *outport_Out = convert_Q31_to_double(pTDT1_FiP32->Out);
}
...
static void x2c_DT1_FiP32_C__Ending(scicos_block* block) {
    DT1_FIP32 *pTDT1_FiP32;
    pTDT1_FiP32 = GetWorkPtrs(block);
    scicos_free(pTDT1_FiP32);
}


static void x2c_DT1_FiP32_C__StateUpdate_f(scicos_block* block) {
...
static void x2c_DT1_FiP32_C__StateUpdate(scicos_block* block) {
    if (GetNevIn(block) > 0) {
        ...
        */
        x2c_DT1_FiP32_C__StateUpdate_f(block);
    }
}
